...
class Hmm(object):
    ...
    def __compute_B(self):
        ...
        for t in range(self.T):
            for y in range(self.K):
                self.b[t, y] = self.eta[y, int(self.data[t, y])]
    ...
    def EM(self, compute_likelihood  = False, max_iter = 1000, precision = 1e-4 ):
        ...
        for i in range(max_iter):
            ...
            # maximimization
            self.pi = np.fmax(self.cond_proba[0,:], min_pi)
            self.pi /= self.pi.sum()
            self.a = np.fmax( self.joined_cond_proba.sum(0)/ self.joined_cond_proba.sum((0,1)), min_a)
            mean_cond_proba = self.cond_proba.sum(0)
            self.eta = np.fmax((self.cond_proba[:,:,None]*self.data[:,None,:]).sum(0) / mean_cond_proba [:,None], min_eta)
