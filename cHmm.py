...
class CHmm(object):
    def __init__(self, data, hmm_type = 'log-scale', hidden_states = 5 ,obs_states = 8):
        ...
        self.C = data.shape[1] # number of coupled HMM (in the case of two HMMs, C=2)
    ...
    def __compute_B(self, data):
        ...
        for t in range(T):
            self.b[t, :] = [self.eta[y, int( data[t, :])] for y in range(self.K)]
    ...
    def EM(self, compute_likelihood  = False, max_iter = 200, precision = 1e-4 ):
        ...
        for i in range(max_iter):
            ...
            self.eta = (self.cond_proba[:,:,None]*self.train_data[:,None,:]).sum(0) / self.cond_proba.sum(0)[:,None]
